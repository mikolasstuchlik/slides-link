...
final class EditorCodeManipulator {
    ...
    func saveUpdatesToSourceCode() -> [String: Result<Void, Swift.Error>] {
        ...
        for slide in knownSlides {
            result[slide.name] = Result {
                ...
                if let regexResult = try EditorCodeManipulator.regex.firstMatch(in: content) {
                    content.replaceSubrange(regexResult[EditorCodeManipulator.yReference].range, with: "\(slide.offset.dy)")
                    content.replaceSubrange(regexResult[EditorCodeManipulator.xReference].range, with: "\(slide.offset.dx)")
                }
    ...
    private func loadFilePathsForNames() -> [String: String] {
        ...
        while let object = enumerator?.nextObject() as? String {
            ...
            if let regexResult = try? EditorCodeManipulator.regex.firstMatch(in: content) {
                let name = "\(regexResult[EditorCodeManipulator.nameReference])"
                result[name] = object
            }
        }
